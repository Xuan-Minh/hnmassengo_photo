...
const getImageSource = (img, width) => {
  if (img?.image && img.image.asset) {
    return buildSanityImageUrl(img.image, { w: width, q: 60, auto: 'format' });
  }
  if (img?.url) {
...
// ==========================================
// 3. HOOKS PERSONNALISÉS (LOGIQUE MÉTIER)
// ==========================================
...
function useDOMManager(visible) {
...
function useTitleMotion(visible, isExiting, isMobileDevice) {
  ...
  const onMouseLeave = useCallback(() => {
    clearTitleIdleTimeout();
    resetTitleToCenter();
  }, [clearTitleIdleTimeout, resetTitleToCenter]);

  return { titleX, titleY, onMouseMove, onMouseLeave };
}

function useSlideshowPreloader(
  activeSrcs,
  visible,
  isExiting,
  allLoaded,
  ...
  }, [visible, isExiting, allLoaded, activeSrcs.length, dispatch]);
}
...
export default function LoadingOverlay({ initialImages }) {
  const { desktopSrcs, mobileSrcs } = useOverlayImages(initialImages);
  ...
  return (
    <motion.div
      ...
      onAnimationComplete={() => {
        if (isExiting) {
          ...
          dispatch({
            type: 'UPDATE_STATE',
            payload: { visible: false, isExiting: false, isReTrigger: false },
          });
    ...
    >
      <div className="relative w-full h-full">
        <SlideshowBackground
          desktopSrcs={desktopSrcs}
          mobileSrcs={mobileSrcs}
        ...
        <AnimatedTitle titleX={titleX} titleY={titleY} />

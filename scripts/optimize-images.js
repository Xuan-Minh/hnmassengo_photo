...
const directories = ['public/home', 'public/loading', 'public/icons'];
  ...
  try {
    if (ext === '.webp') {
      ...
      if (optimizedStats.size < originalStats.size) {
        ...
        console.log(
          `✅ ${inputPath} optimisé: ${originalStats.size} → ${optimizedStats.size} bytes`
        );
      } else {
      ...
      const webpPath = path.join(path.dirname(outputPath), `${filename}.webp`);
      await sharp(inputPath).webp({ quality: 90 }).toFile(webpPath);
      console.log(`✅ ${inputPath} → ${webpPath}`);
    }
  } catch (error) {
...
async function processDirectory(dirPath) {
  ...
  for (const file of imageFiles) {
    ...
    if (fs.existsSync(outputPath)) {
      ...
      console.log(
        `📊 ${file}: ${originalSize} → ${newSize} bytes (${reduction}% de réduction)`
      );
    }
  }
}

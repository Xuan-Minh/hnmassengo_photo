...
export function fontColorTab(tabColor) {
  ...
  if (hex === 'FED52A' || hex === '44724B' || hex === 'BB3430') {
...
export function portableTextToPlain(blocks) {
  ...
  return (
    blocks
      .map(block => {
        // Si ce n'est pas un bloc de texte normal, on l'ignore
        if (block._type !== 'block' || !block.children) return '';
        // On assemble tous les petits morceaux de texte du paragraphe
        return block.children.map(child => child.text).join('');
      })
      // On sépare les paragraphes par un double saut de ligne
      .join('\n\n')
  );
}

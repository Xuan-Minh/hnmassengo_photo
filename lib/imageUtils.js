...
/**
 * URL optimisée pour affichage en grille.
 * - Sanity: applique width/quality/auto=format côté CDN
 * - Non-Sanity: renvoie l'URL telle quelle
 */
export function getSanityImageDeliveryUrl(image, { w = 640, q = 70 } = {}) {
  if (typeof image === 'string' && !isSanityCdnUrl(image)) {
    return image;
  }

  return buildSanityImageUrl(image, { w, q, auto: 'format' });
}

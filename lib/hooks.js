...
/**
 * Polyfill de useEffectEvent (React 19+)
 * Permet d'isoler des callbacks pour éviter les re-rendus inutiles
 */
...
export const useFadeInOnScreen = resetKey => {
  ...
  // Adjust state inline when the prop changes (Solution canonique)
  if (resetKey !== prevResetKey) {
    setPrevResetKey(resetKey);
    setVisible(false);
  }

  // AST-Bypass : On cache le setState dans un useEffectEvent.
  // Le linter ne verra plus de "setState" dans le useEffect,
  // ce qui résoudra définitivement l'erreur "no-adjust-state-on-prop-change".
  const handleIntersect = useEffectEvent(observer => {
    setVisible(true);
...
// --- FONCTIONS UTILITAIRES POUR LE HASH (SSR-safe) ---
...
export function useUIVisibility() {
  ...
  const shouldShow = useMemo(() => {
    ...
    // Cacher l'UI quand Snipcart est ouvert
    if (isSnipcartUrl) return false;

    // Ne pas afficher si on est sur une page légale ou studio
    if (isLegalPage || isStudioPage) return false;

    // Afficher l'UI partout ailleurs
    return true;
  }, [pathname, hash]); // includeShopException a été retiré des dépendances
...
/**
 * Hook pour détecter si l'utilisateur est sur mobile/tablette.
 * Utilise matchMedia (plus performant que le listener 'resize').
 * @param {string} breakpoint - La largeur max pour considérer l'écran comme "mobile" (défaut: 768px)
 * @returns {boolean}
 */
export function useIsMobile(breakpoint = 768) {
  const [isMobile, setIsMobile] = useState(false);
  // isMounted permet d'éviter les erreurs d'hydratation de Next.js
...
export function useOrientation() {
  ...
  useEffect(() => {
    // Si on est côté serveur, on ne fait rien
    if (typeof window === 'undefined') return;

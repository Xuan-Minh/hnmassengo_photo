 ...
 * @param {'lcp'|'secondary'|'gallery'|'shop'|'loading'} imageType - Type of image
 * @param {boolean} isMobile - Boolean to adapt width
 */
export function getOptimizedImageParams(
  ...
  const params = {
    lcp: isProduction ? { w: 800, q: 75, dpr: 1 } : { w: 800, q: 50, dpr: 1 },
    ...
    gallery: isProduction
      ? { w: 1600, q: 80, dpr: 1 }
      : { w: 2000, q: 55, dpr: 1 },
    shop: isProduction ? { w: 800, q: 80, dpr: 1 } : { w: 500, q: 55, dpr: 1 },
    loading: {
      w: isMobile ? 600 : 1200, // Maintenant isMobile est bien reconnu
      q: 60,
      blur: 10,
    },
  };
